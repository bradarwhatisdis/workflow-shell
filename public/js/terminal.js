...
import { getSessionToken } from './modules/api.js';

export const termThemePresets = {
...
let term;
let ws = null, termReady = false, buffer = [], reconnectAttempt = 0;
let fitAddon;
...
function connectWs() {
  ...
  ws.onopen = function () {
    ...
    reconnectAttempt = 0;
    for (let i = 0; i < buffer.length; i++) ws.send(JSON.stringify(buffer[i]));
    buffer = [];
    term.focus();
  };
...
export function initTerminal({ onActivity } = {}) {
  ...
  onActivityCallback = onActivity || null;

  const savedTheme = localStorage.getItem('wfs-terminal-theme') || 'default';
  const initialTheme = termThemePresets[savedTheme] || termThemePresets.default;
  ...
  const origFit = fitAddon.fit ? fitAddon.fit.bind(fitAddon) : null;
  fitAddon.fit = function () {
    if (origFit) {
      try { origFit(); return; } catch (e) { /* ignore */ }
    }
    const cols = Math.max(40, Math.floor((term.element.offsetWidth - 14) / 9));
    const rows = Math.max(10, Math.floor((term.element.offsetHeight - 4) / 19));
